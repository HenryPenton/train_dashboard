...
describe('NoTrainsCard', () => {
  ...
  test('renders no trains message with station names and accessibility features', () => {
    ...
    expect(screen.getByText(/No services currently running between/)).toBeInTheDocument();

    // Check for Real Time Trains link
    const realTimeTrainsLink = screen.getByRole('link', { name: /Check Real Time Trains/i });
