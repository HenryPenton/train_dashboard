...
jest.mock("../StatusBar", () => {
  return function MockStatusBar({ backgroundColor }: { backgroundColor: string }) {
    return <div data-testid="status-bar" style={{ backgroundColor }} />;
  };
});

jest.mock("../StatusBadge", () => {
  return function MockStatusBadge({ status, severity }: { status: string; severity: number }) {
    return <div data-testid="status-badge" data-status={status} data-severity={severity}>{status}</div>;
  };
});
