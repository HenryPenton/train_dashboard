...
jest.mock("../RouteStep", () => {
  return function MockRouteStep({ leg, stepNumber, isLast }: { leg: RouteLeg; stepNumber: number; isLast: boolean }) {
    return (
      <div data-testid={`route-step-${stepNumber}`}>
        Step {stepNumber}: {leg.mode} from {leg.departure} to {leg.arrival} {isLast ? "(last)" : ""}
      </div>
    );
  };
});
...
describe("RouteSteps", () => {
  ...
  it("renders route steps with proper accessibility attributes and content", () => {
    ...
    expect(screen.getByText("Step 3: tube from Oxford Circus to Liverpool Street (last)")).toBeInTheDocument();
  });
