...
describe("JourneyMetrics", () => {
  it("renders journey metrics with proper accessibility attributes", () => {
    ...
    const durationElement = screen.getByLabelText(
      `Journey duration: ${testDuration} minutes`,
    );
  ...
  it("renders without fare when not provided", () => {
    ...
    render(<JourneyMetrics duration={testDuration} arrival={testArrival} />);

    // Check duration and arrival are present
    expect(
      screen.getByLabelText(`Journey duration: ${testDuration} minutes`),
    ).toBeInTheDocument();
