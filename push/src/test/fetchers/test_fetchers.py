...
def test_fetch_rail_departures():
    with patch("src.fetchers.fetchers.requests.get") as mock_get:
        mock_resp = MagicMock()
        ...
        mock_get.assert_called_once_with(
            "http://localhost:8000/rail/departures/AAA/to/BBB"
        )
...
def test_fetch_best_route():
    with patch("src.fetchers.fetchers.requests.get") as mock_get:
        ...
        mock_get.assert_called_once_with("http://localhost:8000/tfl/best-route/AAA/BBB")
        mock_resp.raise_for_status.assert_called_once()
...
def test_fetch_tube_line_statuses():
    with patch("src.fetchers.fetchers.requests.get") as mock_get:
        ...
        mock_get.assert_called_once_with("http://localhost:8000/tfl/line-status")
        mock_resp.raise_for_status.assert_called_once()
