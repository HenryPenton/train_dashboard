...
export default function Settings() {
  ...
  const handleTubeDepartureChange = (e: React.ChangeEvent<HTMLInputElement>) => {
          ...
          <div className="space-y-4">
            ...
            {config?.tfl_line_status?.enabled && (
              <InputField
                ...
                onChange={(e) => updateTflLineStatusImportance(parseInt(e.target.value) || 1)}
                placeholder="1"
              />
            )}
          </div>
          ...
          {config?.rail_departures && config.rail_departures.length > 0 && (
            <div className="space-y-2">
              {config.rail_departures.map((departure, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-[#2a2d35] rounded">
                  <div>
                    <span className="font-semibold">{departure.origin} → {departure.destination}</span>
                    <span className="text-gray-400 ml-2">({departure.originCode} → {departure.destinationCode})</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <InputField
                      label=""
                      value={departure.importance?.toString() || "1"}
                      onChange={(e) => updateDepartureImportance(index, parseInt(e.target.value) || 1)}
                      className="w-20"
                    />
                    <Button
                      variant="danger"
                ...
                <div key={index} className="flex items-center justify-between p-3 bg-[#2a2d35] rounded">
                  ...
                  <div className="flex items-center gap-2">
                    <InputField
                      ...
                      onChange={(e) => updateRouteImportance(index, parseInt(e.target.value) || 1)}
                      className="w-20"
                    />
                    <Button
                      variant="danger"
              ...
              {config.tube_departures.map((departure, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-[#2a2d35] rounded">
                  <div>
                    ...
                    <span className="text-gray-400 ml-2">({departure.stationId})</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <InputField
                      label=""
                      value={departure.importance?.toString() || "1"}
                      onChange={(e) => updateTubeDepartureImportance(index, parseInt(e.target.value) || 1)}
                      className="w-20"
                    />
                    <Button
                      variant="danger"
